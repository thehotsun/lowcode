<template>
  <el-form ref="ruleForm" :model="form" label-width="130px" style="padding-bottom: 20px">
    <el-form-item label="树数据源" prop="isDataModel">
    ...
    <el-form-item v-if="form.isDataModel" :label="sqlLabel" :prop="sqlField">
      <el-input v-model="form[sqlField]" :autosize="{ minRows: 2, maxRows: 4 }" type="textarea" readonly style="width: 686px"></el-input>
      <el-button type="text" @click="handleSqlConfigEdit">编辑</el-button>
    </el-form-item>
    <el-form-item v-if="form.isDataModel" :label="orderExprLabel">
      <el-input v-model="form[orderExprField]" :autosize="{ minRows: 2, maxRows: 6 }" type="textarea" maxlength="200" />
    </el-form-item>

    <sqlConfig ref="sqlConfig" :form="form" :sql-field="sqlField" :order-expr-field="orderExprField"></sqlConfig>

    <el-form-item v-if="!form.isDataModel" :label="requestUrlLabel">
      <el-input v-model="form[requestUrlField]" style="width: 180px" placeholder="请输入接口地址"></el-input>
...
export default {
  components: { sqlConfig, interfaceConfig },
  props: {
    form: {
      type: Object,
      default() {
